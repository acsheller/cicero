...
# Item and item loader -- caveot -- get it working later
from environment.item import ItemsLoader

from environment.items_retrieval import ItemsRetrievalMind
from environment.items_selection import ItemsSelectorMind

from environment.LLM import LLMRater
from environment.memory import NewsMemory
from environment.reward_perturbator import RewardPerturbatorMind
from environment.reward_shaping import RewardShapingMind
from environment.users import UsersLoader

from environment.mind.news_loader import NewsLoader

class Simulatio4RecSysMind(gym.Env):
    def __init__(
        self,
        render_mode: str,
        items_loader: ItemsLoader,
        users_loader: UsersLoader,
        items_selector: ItemsSelectorMind,
        reward_perturbator: RewardPerturbatorMind,
        items_retrieval: ItemsRetrievalMind,
        reward_shaping: RewardShapingMind,
        llm_rater: LLMRater,
        ...
        items_interact (list of tuple of integers), is a list of News IDs seen and the rating assigned by the user (corresponding to user_id)
        ...
        """
        We have one action for each object in the dataset, one action correspond to recommend one item
        """
        self.action_space = spaces.Discrete(self.num_items)

        """
        The following dictionary maps abstract action to films ids, such that each action correspond to recommending 
    ...
    def step(self, action: int):
        ...
        """
        Use the itemsLoader to load curr_item, which is an object crresponding to the id item_id
        Or could be a News Item 
        """
        ...
        """
        The next step is to retrieve from the list of all items seen a smaller list of relevant items. The relevance from the Object
            ...
            df = pd.DataFrame(
                {
                    ...
                    "item_id": np.stack(list(self._items_interact))[-1, 0],
