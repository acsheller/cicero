...
class LLMExllama(LLM):
    ...
    def request_rating_1_10(self, system_prompt, dialog) -> Tuple[str, str]:  # TODO need fix
        prompt = self.encode(system_prompt, dialog)
        if self.use_cache:
            return prompt, self.generate_reuse_simple(
                self.generator_rating_1_10, prompt, max_new_tokens=1
            )
        else:
    ...
    def generate_description_embedding(self, description: str) -> np.ndarray:
        inputs = self.tokenizer.encode(description)
        self.generator_explanation.gen_begin_reuse(inputs)
        embedding = self.generator_explanation.gen_single_token().detach().numpy().flatten()
        return embedding
