...
class NewsMemory:
    ...
    user_to_seen_news_articles: typing.Dict[
        int, typing.Dict[int, typing.List[UserNewsInteraction]]
    ]

    def __init__(self, items_loader):
        self.user_to_seen_news_articles = {}
    ...
    def update_memory(
        ...
        Return:
            None
        """
        if user_id not in self.user_to_seen_news_articles:
            ...
            if item_id in self.user_to_seen_news_articles[user_id]:
                self.user_to_seen_news_articles[user_id][item_id].append(
                    UserNewsInteraction(
                        ...
                        len(self.user_to_seen_news_articles[user_id][item_id]) + 1,
                    )
                )
            else:
                self.user_to_seen_news_articles[user_id][item_id] = [
                    UserNewsInteraction(
    ...
    def _initialize_user(self, user_id: int):
        ...
        Return:
            None
        """
        self.user_to_seen_news_articles[user_id] = {}
    ...
    def _get_items_ids_and_interactions(self, user_id: int):
        ...
        if user_id not in self.user_to_seen_news_articles:
            self._initialize_user(user_id)
            return [], []
        for item_id, interaction in self.user_to_seen_news_articles[user_id].items():
    ...
    def delete_user_item(self, user_id: int, item_id: int):
        ...
        """
        del self.user_to_seen_news_articles[user_id][item_id]
    ...
    def delete_last_user_item_interaction(self, user_id: int, item_id: int):
        ...
        Args:
            ...
            item_id (integer): id of the item we want to delete
        """
        self.user_to_seen_news_articles[user_id][item_id].pop()
        if self.user_to_seen_articles[user_id][item_id] == []:
    ...
    def get_num_interaction(self, user_id: int, item_id: int):
        ...
        if type(item_id) is int:
            if int(item_id) not in self.user_to_seen_news_articles[user_id]:
                return 0
        elif item_id not in self.user_to_seen_news_articles[user_id]:
            return 0
        return self.user_to_seen_news_articles[user_id][item_id][-1].num_watches
