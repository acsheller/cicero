...
class ItemsSelectorMind(ABC):
    ...
    @abstractmethod
    def select(
        self, items: typing.List[News], ratings: typing.List[float]
    ) -> typing.Tuple[typing.List[News], typing.List[float]]:
...
class GreedySelectorMind(ItemsSelectorMind):
    """
    ...
    def select(
        ...
        """
        Select greedly one item based on the rating, in this case it makes a deterministic choice and picks the news item
        with highest rating, in case there is more than one we pick the one with higher vote_average

        Args:
            items (list of News article): the list of items the user has to select from
...
class GreedySelectorRandomMind(ItemsSelectorMind):
    """
    Object that is responsable to select Movies, given a list of items, with predicted rating (from LLM), selects the one that the user will see.
    ...
    def __init__(self, p=0.9, seed=42):
        self.greedy_selector = GreedySelectorMind(seed)
        super().__init__(seed)
        self.p = p
...
class RandomSelectorMind(ItemsSelectorMind):
    """
    Object that is responsable to select Movies, given a list of items, with predicted rating (from LLM), selects the one that the user will see.
